...
bool IsTriggerSizeOpenTime(char direction,double wvad,double spread){
...
bool IsTriggerSizeCloseTime(char direction,mdPrice_band *now_price,mdPrice_band *pre_price,int multiple
...
bool IsDownTime(mdPrice_band *now_price,mdPrice_band *pre_price,int spread,int multiple){
...
bool IsUpTime(mdPrice_band *now_price,mdPrice_band *pre_price,int spread,int multiple)
{
...
void StartAndStopFun(Parameter_band *param,VolumeTrendOther3Info *info,int param_index){
	...
	info->tick_cur_num = 0;
	info->tick_max_price = 0;
	info->tick_min_price = 0;
	info->tick_open_volume = 0;
	info->tick_open_price = 0;
...
BandAndTriggerSizeRetStatus GetMdData(Parameter_band *param,VolumeTrendOther3Info *info,int param_index){
	...
	int limit_wvad_period = param->m_Param[param_index+1].spread;
	...
	if (info->tick_open_volume ==0)
	{
		info->tick_open_volume = info->cur_price.Volume;
	}
	if (info->tick_min_price ==0 || info->tick_min_price > lastprice)
	{
		info->tick_min_price = lastprice;
	}
	if (info->tick_max_price ==0 || info->tick_max_price < lastprice)
	{
		info->tick_max_price = lastprice;
	}
	if (info->tick_open_price ==0)
	{
		info->tick_open_price = lastprice;
	}
	int diff_volume = info->cur_price.Volume - info->tick_open_volume;
	double diff_close_open_price = lastprice - info->tick_open_price;
	double diff_min_max = info->tick_max_price - info->tick_min_price; 
	double tmp_wvad = 0;
	if (diff_min_max ==0)
	{
		tmp_wvad = 0;
		...
		tmp_wvad = (diff_close_open_price/diff_min_max)*diff_volume;
	}
	info->tick_cur_num +=1;
	if (info->tick_cur_num >= limit_wvad_period)
	{
		info->tick_cur_num = 0;
		info->tick_max_price = 0;
		info->tick_min_price = 0;
	...
	ret.isTrendOpenTime = IsOpenTime(middle_val,sd_val,param,info,param_index,tmp_wvad);
	ret.isTrendCloseTime = IsCloseTime(middle_val,sd_val,rsi_data,param,info,param_index);

	return ret;
}

bool IsOpenTime(double middle_val,double sd_val,Parameter_band *param,VolumeTrendOther3Info *info,int param_index,double wvad){
	...
	bool is_trigger_open = IsTriggerSizeOpenTime(info->direction,wvad,spread);

	return is_trigger_open;
}

bool IsCloseTime(double middle_val,double sd_val,double rsi_val,Parameter_band *param,VolumeTrendOther3Info *info,int param_index){
