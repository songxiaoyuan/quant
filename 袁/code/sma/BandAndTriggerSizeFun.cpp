...
double GetSMAData(double price,double pre_sma_val,int period,int weight){
	if (period <=1)
	{
		return price;
	}
	double ret = ((period - weight)*pre_sma_val + weight*price)/period;
	return ret;
}
...
BandAndTriggerSizeRetStatus GetMdData(Param *param,VolumeTrendOther3Info *info){
	...
	if (info->prices_queue.size() < period )
	{
		...
		info->pre_sma_val = GetSMAData(lastprice,info->pre_sma_val,info->prices_queue.size(),weight);
	...
	middle_val =  GetSMAData(lastprice,info->pre_sma_val,period,weight);
	info->pre_sma_val = middle_val;
