...
void BandAndTriggerSize::clearVector(){
	Parameter_band *param = &interface_param_;
	VolumeTrendOther3Info *info = &TriggerSize3_info_;

	memset(&info->pre_price,0,sizeof(mdPrice_band));
	memset(&info->cur_price,0,sizeof(mdPrice_band));
	...
	int param_index =0;
	//spread
	param->m_Param[param_index].spread = 2000;
	//diff_volume
	...
	//tick num
	param->m_Param[param_index+1].spread =120;
	...
	param->m_Param[param_index+1].cancelEdge = 100;

	//small the limit sd
	param->m_Param[param_index+1].edgebWork = 50;
